...
max.function.partitioned4 <- function(data.df, id, supp.labs, labnms) {
  ...
  temp$Month <- factor(temp$Month)
  temp$common <- factor(temp$common, c("DX-CS-CV","DX-CS-CT","DX-CS-NT","DX-R-CV","DX-R-CT","DX-R-NT","DX-R-R"))
  ...
  # windows()
  plot.max <- ggplot(temp, aes(x=Month, y=avg.max, fill=Pothole))+
    geom_bar(stat="identity", position="identity", alpha=0.25, color="black", size=0.15)+
